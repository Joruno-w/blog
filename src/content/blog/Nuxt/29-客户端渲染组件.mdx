...
英文是 Island Components. 岛屿组件的核心思想是**不做水合**。

传统的 SSR 应用中，服务器会生成完整的首屏页面，然后客户端拿到首屏的 html 后再做水合操作，将整个页面转为可以交互的实例。

![](https://raw.githubusercontent.com/wangshengliang-wsl/blog/main/img/20251015142134562.png)
...
| 组件         | 核心目标                                                       |
| :----------- | :------------------------------------------------------------- |
| `ClientOnly` | 完全跳过服务端渲染，确保子组件仅在客户端渲染                   |
| `NuxtIsland` | 服务端渲染成静态内容，客户端直接展示这部分静态内容，不做水合。 |
