...
基于 [ofetch](https://www.npmjs.com/package/ofetch) 封装的工具。ofetch 这个工具也是 Nuxt3 官方团队成员推出的，特点：
...
// 使用 ofetch
await ofetch('https://jsonplaceholder.typicode.com/404').catch(
  (err) => err.data
)
```
...
1. data：存储异步数据请求的结果，并且这是一个响应式数据。
2. status：表示请求的当前状态。
3. error：存储请求过程中发生的错误。
4. refresh：是一个函数，用于重新触发数据请求。当数据需要实时更新时，可以调用 refresh( ) 强制刷新数据。
5. clear：是一个函数，用于清除当前数据的缓存。

下表是两者之间的核心区别：

| 特性         | $fetch              | useAsyncData                       |
| ------------ | ------------------- | ---------------------------------- |
| 是否响应式   | 否，返回普通数据    | 是，返回响应式数据                 |
| 自动缓存     | 否                  | 是，默认缓存到客户端，避免重复请求 |
| 支持错误处理 | 是，需手动try-catch | 是，内置 error 状态支持            |
| 支持加载状态 | 否，需要手动实现    | 是，内置 status 可以获取请求状态   |
...
- $fetch: 方法灵活性高，可以**在任何地方使用**。
- useAsyncData: 为服务端渲染（SSR）或组件挂载之前的数据获取场景设计，返回响应式数据以及一系列工具函数。如果是在组件已经挂载后使用，Nuxt 会给出一个警告：Component is already mounted, please use $fetch instead.

**useFetch**

语法糖，针对 useAsyncData 和 $fetch 做了一层封装，该组合式函数会根据 URL 以及 fetch 选项**自动生成 key**，不需要开发人员手动提供 key.
