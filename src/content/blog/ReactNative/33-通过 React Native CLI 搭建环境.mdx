...
> 备注 2: 如果是 _macOS_ 系统电脑，可以搭建 _iOS_ 和 _Android_ 的环境，但是如果 _Windows_ 系统电脑，只能搭建 _Android_ 的环境。

> 备注 3: 本次演示步骤示例，_Development OS_ 为 _macOS_，_target_ 为 _Android_
...
首先下载安装 [_Android Studio_](https://developer.android.google.cn/studio/)
...
安装好 _Android Studio_ 后，接下来需要安装 _Android SDK_，根据官网的描述，目前编译 _React Native_ 应用需要的是 _Android 11 (R)_ 版本的 _SDK_，可以在 _Android Studio_ 的 _SDK Manager_ 中选择安装各版本的 _SDK_。
...
根据官网的描述，接下来我们需要安装上图中框选的组件，但是第一次是无法选择的，因为没有选择 _Android SDK_ 的 _Location_，点击右侧的 _Edit_，先安装一个默认的 _Android SDK_ 版本，目前默认的是 _Android API 33_，展开 _Android 11 (R)_，勾选上图中框选的组件
